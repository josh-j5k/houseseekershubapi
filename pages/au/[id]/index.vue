...
function showEditModal() {
    ...
    form.title = listings.data[currentIndex.value].title
    form.location = listings.data[currentIndex.value].location
    form.price = listings.data[currentIndex.value].price
    form.description = listings.data[currentIndex.value].description
    form.property_status = listings.data[currentIndex.value].propertyStatus
    form.property_type = listings.data[currentIndex.value].propertyType
    setTimeout(() => {
        ...
        total.value = listings.data[currentIndex.value].images.length
    }, 100)
}
...
function deletePhoto(ev: MouseEvent) {
    const btn = ev.currentTarget as HTMLButtonElement
    const btnIndex = parseInt(btn.value)
    form.deletedImages.push(listings.data[currentIndex.value].images[btnIndex])
    listings.data[currentIndex.value].images.splice(btnIndex, 1)
}
...
<template>
    ...
    <div class="pb-12 py-4 lg:h-[calc(100vh-65px)] lg:overflow-hidden">
        <div class="max-w-7xl bg-white shadow-sm  h-full w-full sm:rounded-lg mx-auto sm:px-6 lg:px-8">
            <div v-if="!listings.data" class="p-6 text-center text-gray-900">
